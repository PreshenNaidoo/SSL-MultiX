...
# Define the model with model-subclassing
class PretextModel(keras.Model):
    def __init__(self):
        super().__init__()

        self.augmenter = get_augmenter_zoom_trans(**AUG_ROTATION)
        # With rescaling because we're not using rescaling inside the augmenter with this pre-text task.
        ...
        self.projection_head_with_classification = keras.Sequential(
            [
                keras.Input(shape=encoder_output_shape),#(shape=(16,16,1024)), #output shape of the encoder
                layers.Flatten(),
                layers.Dense(128, activation="relu"),
                layers.Dense(128),
                layers.Dense(36, activation='softmax') #36 classes (0 to 360, step 10 degrees)
            ],
            name="Project_Head",
        )

        self.encoder.summary()
        self.projection_head_with_classification.summary()

    def compile(self, optimizer, **kwargs):
        super().compile(**kwargs)

        self.optimizer = optimizer
        self.loss = keras.losses.SparseCategoricalCrossentropy(reduction=tf.keras.losses.Reduction.NONE)
        self.loss_tracker = keras.metrics.Mean(name="loss")
        self.accuracy = keras.metrics.SparseCategoricalAccuracy(name="acc")
    ...
    def train_step(self, data):
        ...
        #add slight translation and zoom to it
        images = self.augmenter(images, training=True)

        #build rotation labels on the fly
        batch_size = tf.shape(data)[0]
        indices = tf.random.uniform(shape=(batch_size,), minval=0, maxval=35, dtype=tf.int32)
        angles = (tf.cast(indices, dtype=tf.float32) * 10 * 2 * TF_PI) / 360.0
        rotated_images = tfa.image.rotate(images, tf.transpose(angles))

        with tf.GradientTape() as tape:
            features = self.encoder(rotated_images, training=True)
            # The representations are passed through a projection mlp
            predictions = self.projection_head_with_classification(features, training=True)
            model_loss = self.loss(indices, predictions)
        gradients = tape.gradient(
            model_loss,
            self.encoder.trainable_weights + self.projection_head_with_classification.trainable_weights,
        )
        ...
        self.loss_tracker.update_state(model_loss)
        self.accuracy.update_state(indices, predictions)
    ...
    def test_step(self, data):
        ...
        features = self.encoder(rotated_images, training=False)
        # The representations are passed through a projection mlp
        predictions = self.projection_head_with_classification(features, training=False)
        model_loss = self.loss(indices, predictions)
...
def rotation_pretraining(exp_folder, train_dataset, unlabelled_val, num_unlabelled, batch_size_unlabelled, num_epochs):
    ...
    # Pretraining
    pretraining_model = PretextModel()
    ...
    pretraining_model.encoder.save_weights(os.path.join(contrastive_training_path, f'last_epoch_encoder.h5'))
...
def run_ssl(mixed_precision, single_gpu_unlabelled_batch_size, experiment_folder, unlabelled_dataset_path, percentage_unlabelled):
    ...
    with strategy.scope():
        ...
        pretraining_history, pretraining_model, ellapsed_time = rotation_pretraining(experiment_folder,
                                                                                     unlabelled_train_dataset,
                                                                                     unlabelled_val, num_unlabelled,
