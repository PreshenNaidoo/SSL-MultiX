...
from inference import infer_test_segmentation, prepare_inference_dataset
import time
...
def get_model_for_reconstruction(model_file):
    encoder, skips = get_unet_encoder((IMAGE_SIZE, IMAGE_SIZE, IMAGE_CHANNELS), dropout_prob=0)
    encoder.load_weights(model_file, by_name=True)
    model = create_unet_model_reconstructive(encoder=encoder,
                                         skip_connections=skips,
                                         input_shape=(IMAGE_SIZE, IMAGE_SIZE, IMAGE_CHANNELS),
                                         num_output_channels=IMAGE_CHANNELS,
                                         lock_entire_enc=True,
                                         lock_enc_batch_norm_layers=True,
                                         lock_all_batch_norm_layers=False)
    return model

def get_model_for_test_reconstruction(model_file):
    model = None
    if BACKBONE == BACKBONES[0]:
        encoder, skips = get_unet_encoder_with_rescaling((IMAGE_SIZE, IMAGE_SIZE, IMAGE_CHANNELS), dropout_prob=0)
        model = create_unet_model_reconstructive(encoder=encoder,
                                             skip_connections=skips,
                                             input_shape=(IMAGE_SIZE, IMAGE_SIZE, IMAGE_CHANNELS),
                                             num_output_channels=IMAGE_CHANNELS,
                                             lock_entire_enc=False,
                                             lock_enc_batch_norm_layers=True,
                                             lock_all_batch_norm_layers=False)
...
class PatchReconstructionModel(keras.Model):
    ...
    def train_step(self, data):
        ...
        modified_frames = tf.cast(modified_frames, tf.float32) / 255.0
        curr_frames1 = tf.cast(images, tf.float32) / 255.0

        with tf.GradientTape() as tape:

            features_1 = self.encoder(modified_frames, training=True)
            reconstructive_loss = self.reconstructive_loss(curr_frames1, features_1)
        ...
        self.contrastive_loss_tracker.update_state(reconstructive_loss)
...
def patch_pretraining(exp_folder, train_dataset, unlabelled_val, num_unlabelled, batch_size_unlabelled, num_epochs):
    contrastive_training_path = os.path.join(exp_folder, 'reconstructive_training')
    learning_curve_path = os.path.join(contrastive_training_path, 'learning_curve.png')
    csv_save_path = os.path.join(contrastive_training_path, 'epoch_history.csv')
    model_save_path_encoder_weights = os.path.join(contrastive_training_path, 'best_encoder_weights.h5')
    model_save_path_encoder = os.path.join(contrastive_training_path, 'best_encoder.h5')
...
def run_ssl_reconstruct(mixed_precision, single_gpu_unlabelled_batch_size, experiment_folder, unlabelled_dataset_path, percentage_unlabelled):
    if not os.path.exists(experiment_folder):
        os.makedirs(experiment_folder)
    ...
    print("-------------------------")
    with strategy.scope():
        contrastive_training_path = os.path.join(experiment_folder, 'reconstructive_training')
        # if not os.path.exists(contrastive_training_path):
...
def main():
    ...
    main_folder = 'run_split'
    main_folder = f'{main_folder}_{BACKBONE}_{LOSS}_{ssl_mixed_precision}_batch{single_gpu_batch_size}'

    train_ssl = True                   #train a model on a pretext task using only unlabelled images
    train_ssl_reconstruct = False
    ...
    # SSL training
    if train_ssl_reconstruct:
        for perc_unlabelled in percentages_for_unlabelled_training:
            experiment_folder = os.path.join(main_folder, f'Exp_with_{perc_unlabelled}_percent_unlabelled_data')
            run_ssl_reconstruct(mixed_precision=ssl_mixed_precision,
                                single_gpu_unlabelled_batch_size=single_gpu_batch_size,
                                experiment_folder=experiment_folder,
                                unlabelled_dataset_path=unlabelled_dataset_path,
                                percentage_unlabelled=perc_unlabelled)
    ...
    if train_fine_tuned_model or test_fine_tune:
        for perc_labelled in percentages_for_downstream_training:
            for perc_unlabelled in percentages_for_unlabelled_training: #just so we know which pretrained encoder to use
                ...
                tf.random.set_seed(777)  # 2112
                np.random.seed(555)  # 555
                random.seed(143)
